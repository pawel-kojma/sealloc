...
void run_init(run_t *run, bin_t *bin, void *heap) {
  ...
  memset(run->reg_bitmap, 0x0,
         bin->reg_mask_size_bits / 8 + (bin->reg_mask_size_bits % 8 != 0));
}
...
void *run_allocate(run_t *run, bin_t *bin) {
  ...
  uint8_t elems = (uint8_t)(bin->reg_mask_size_bits / 2);
...
void run_deallocate(run_t *run, bin_t *bin, void *ptr) {
  uintptr_t heap = (uintptr_t)run->run_heap;
  size_t reg_idx = ((uintptr_t)ptr - heap) / bin->reg_size;

  // Sanity check
  if (get_bitmap_item(run->reg_bitmap, reg_idx) != STATE_ALLOC_FREE) {
    se_error(
        "Double free or invalid free, Region(heap=%p, nfree=%u, gen=%u, "
        "current_idx=%u), ptr=%p",
        run->run_heap, run->nfree, run->gen, run->current_idx, ptr);
  }

  // Mark as freed
  set_bitmap_item(run->reg_bitmap, reg_idx, STATE_ALLOC_FREE);
}
...
void run_validate_freeable(run_t *run, bin_t *bin, void *ptr) {
  ...
  if (rel_ptr % bin->reg_mask_size_bits != 0) {
    se_error("Unaligned ptr (ptr=%p)", ptr);
  }

  // Check for double free
  if (get_bitmap_item(run->reg_bitmap, bitmap_idx) != STATE_FREE) {
...
bool run_is_empty(run_t *run, bin_t *bin) {
  return run->nfree == bin->reg_mask_size_bits;
}
