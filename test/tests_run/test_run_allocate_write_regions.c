#include <assert.h>
#include <fcntl.h>
#include <sealloc/bin.h>
#include <sealloc/random.h>
#include <sealloc/run.h>
#include <sealloc/utils.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>

#define PAGESIZE 4096

void test_small(void) {
  bin_t *bin;
  run_t *run;
  void *chunks[256];
  void *chunks_m[256];
  char buf[16];
  void *heap = malloc(4096);
  init_splitmix32(1);
  int fd = open("/dev/urandom", O_RDONLY);
  if (fd < 0) {
    printf("fd < 0");
    exit(1);
  }
  bin = (bin_t *)malloc(sizeof(bin_t));
  bin->runcur = NULL;
  bin->run_list = NULL;
  bin->reg_size = 16;
  bin->run_size_pages = 1;
  bin->reg_mask_size_bits = (PAGESIZE / bin->reg_size) * 2;
  run =
      (run_t *)malloc(sizeof(run_t) + BITS2BYTES_CEIL(bin->reg_mask_size_bits));

  run_init(run, bin, heap);
  int elems = run->nfree;
  for (int i = 0; i < elems; i++) {
    chunks[i] = run_allocate(run, bin);
    chunks_m[i] = malloc(16);
    read(fd, buf, 16);
    memcpy(chunks[i], buf, 16);
    memcpy(chunks_m[i], buf, 16);
  }

  for (int i = 0; i < elems; i++) {
    assert(memcmp(chunks[i], chunks_m[i], 16) == 0);
  }
}

void test_medium(void) {
  bin_t *bin;
  run_t *run;
  void *chunks[4];
  void *chunks_m[4];
  char buf[1024];
  void *heap = malloc(4096);
  init_splitmix32(1);
  int fd = open("/dev/urandom", O_RDONLY);
  if (fd < 0) {
    printf("fd < 0");
    exit(1);
  }
  bin = (bin_t *)malloc(sizeof(bin_t));
  bin->runcur = NULL;
  bin->run_list = NULL;
  bin->reg_size = 1024;
  bin->run_size_pages = 1;
  bin->reg_mask_size_bits = (PAGESIZE / bin->reg_size) * 2;

  run =
      (run_t *)malloc(sizeof(run_t) + BITS2BYTES_CEIL(bin->reg_mask_size_bits));

  run_init(run, bin, heap);
  int elems = run->nfree;
  for (int i = 0; i < elems; i++) {
    chunks[i] = run_allocate(run, bin);
    chunks_m[i] = malloc(1024);
    read(fd, buf, 1024);
    memcpy(chunks[i], buf, 1024);
    memcpy(chunks_m[i], buf, 1024);
  }

  for (int i = 0; i < elems; i++) {
    assert(memcmp(chunks[i], chunks_m[i], 1024) == 0);
  }
}

int main(void) {
  test_small();
  test_medium();
  return 0;
}
