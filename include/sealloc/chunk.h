...
#define CHUNK_CHUNK_SIZE_BYTES 4194304  // 2^22 B = 4MB
#define CHUNK_LEAST_REGION_SIZE_BYTES (PAGE_SIZE * 2)
#define CHUNK_NO_NODES_LAST_LAYER \
  (CHUNK_CHUNK_SIZE_BYTES / CHUNK_LEAST_REGION_SIZE_BYTES)
...
// sum((2**(i-1)) * (10 - i) for i in range(1,10))
// Only for tree with depth 10
#define CHUNK_ADDITIONAL_DEPTH_BITS 1013

#define CHUNK_BUDDY_TREE_SIZE_BYTES \
  ((((CHUNK_BUDDY_TREE_SIZE_BITS + CHUNK_ADDITIONAL_DEPTH_BITS) + 7) & ~7) / 8)

typedef struct chunk_state {
  struct chunk_state *fd;
  struct chunk_state *bk;
  uint8_t reg_size_small_medium[CHUNK_NO_NODES_LAST_LAYER];  // Stores reg_size // 16 for small and medium size class
